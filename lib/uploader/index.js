 ...
 * @param   {Object}    {size, file}    The request object
 ...
 * */
function single (provider, { size, file, root, publicPath }, name) {
    ...
    return new Promise((resolve, reject) => {
        ...
        fh.generateFileName(Prov, provider.substring(0, 1), size, name, publicPath)
            .then((filename) => {
                ...
                let targetPath = `${root}/${filename}.${filetype}`;

                fh.mv(tmpPath, targetPath)
                    .then(mvHandler)
                    .catch((err) => {
                        reject(err);
                    });
