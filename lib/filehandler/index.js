    ...
    return new Promise((resolve, reject) => {
        ...
        provider.findOne({ name })
            .then((data) => {
                ...
                let newName = deConflictName({
                    prefix,
                    size,
                    name,
                    sepperator: SEPPERATOR
                }, data.thumbnails[size], root);
...
/**
 * @param   {object}    {prefix, size, name, sepperator}    Components which the name consists of
 * @param   {string[]}  thumbnails                          List of existing thumbnails
 *
 * Based on the name components, generates a name and checks for conflicts in thumbnail
 * @return  {string}    Basic name
 * */
function deConflictName ({ prefix, size, name, sepperator }, thumbnails) {
    let newName;
    let counter = 0;

    while (true) {
        newName = `${prefix}${sepperator}${size}${sepperator}${name}${sepperator}${counter}`;

        if (!exists(newName, thumbnails)) {
            break;
        }
